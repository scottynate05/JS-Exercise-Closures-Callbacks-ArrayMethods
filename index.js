...
function processLength(list, callback) {
  return callback(list.length); /* CODE HERE */
}
...
function processLastItem(stringList, callback) {
  return callback(stringList[stringList.length - 1]) /* CODE HERE */
}
...
function processSum(numberList, callback) {
  return callback(numberList.reduce((numSum, currentValue) => {
    return numSum + currentValue;
  }, 0)); /* CODE HERE */
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(runner) {
    runnersNames.push(runner.last_name + ", " + runner.first_name);
  });
  return runnersNames; /* CODE HERE */
}
...
function firstNamesAllCaps(runners) {
  const capsNames = []
  runners.forEach(function(runner) {
    capsNames.push(runner.first_name.toUpperCase())
  });
  return capsNames; /* CODE HERE */
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter(runners => runners.shirt_size === tShirtSize); /* CODE HERE */
}
...
function tallyUpDonations(runners) {
  const runnersDonation = runners.reduce(function(total, currentValue) {
    return total + currentValue.donation;
  }, 0);
  return runnersDonation; /* CODE HERE */
}
